...
# 2. Static Categorical Mapping Encoders [cite: 1]
GENDER_MAP = {"Female": 0, "Male": 1, "Other": 2}
SUBSCRIPTION_MAP = {"Basic": 0, "Standard": 1, "Premium": 2}

# 3. Enhanced HTML Template with dynamic City Text Input
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <script>
        document.getElementById('predictionForm').addEventListener('submit', async (e) => {
            ...
            formData.forEach((value, key) => {
                // Keep string inputs as strings (like city/gender), map floats/ints otherwise
...
def predict():
    ...
    try:
        ...
        # 1. Read input string from open City text input field [cite: 1]
        raw_city = str(data.get("city", "Other")).strip().lower()
        
        # 2. Dynamic continuous encoding value [cite: 1]
        # Maps any string entry to a stable numerical index value [0-4] matching model expected float inputs 
        encoded_city = abs(hash(raw_city)) % 5
        
        encoded_gender = GENDER_MAP.get(data.get("gender"), 0) [cite: 1]
        encoded_subscription = SUBSCRIPTION_MAP.get(data.get("subscription_type"), 0) [cite: 1]
        
        # Structural layout configuration array [cite: 1]
        features = [
            float(data.get("age", 0)), [cite: 1]
            float(encoded_gender), [cite: 1]
            float(encoded_city), [cite: 1]
            float(data.get("tenure_months", 0)), [cite: 1]
            float(data.get("avg_order_value", 0)), [cite: 1]
            float(data.get("total_orders", 0)), [cite: 1]
            float(data.get("last_purchase_days_ago", 0)), [cite: 1]
            float(data.get("support_tickets", 0)), [cite: 1]
            float(encoded_subscription) [cite: 1]
        ]
        
        input_matrix = np.array([features]) [cite: 1]
        
        prediction = int(model.predict(input_matrix)[0]) [cite: 1]
        probabilities = model.predict_proba(input_matrix)[0].tolist() [cite: 1]
